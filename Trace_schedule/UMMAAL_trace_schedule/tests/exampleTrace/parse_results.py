...
for line in sys.stdin:
	...
	if (time.match(l)):
		...
		if (currentTime == lastTime and ~firstLine): # We use this currentTime and lastTime to ensure that for each value of time we output only the "most updated" one (i.e., we skip all intermediate changes that occur at the same point in time)
			continue
		if (firstLine): # First line of the output file: write simply the column headers (so "Time" and the processor names)
			firstLine = False
		...
		else: # All other lines: write the current time and the state of each processor
			sys.stdout.write(lastTime)
			for n in pName.finditer(lastState):
				procState = n.group(2)
				if (procState == "Idle"):
					procState = "-"
